...
class IndexedFormula(Formula):
	...
	"""
	hits = self._index.get((pred, subj, obj), {}).keys()
	if not hits: return None
...
#        self.statements.append(s)
        ...
        if list is None: self._index[(None, None, obj)]={s : 1}
        else: addIt(s, list)

        list = self._index.get((None, subj, None), None)
        if list is None: self._index[(None, subj, None)]={s : 1}
        else: addIt(s, list)

        list = self._index.get((None, subj, obj), None)
        if list is None: self._index[(None, subj, obj)]={s : 1}
        else: addIt(s, list)

        list = self._index.get((pred, None, None), None)
        if list is None: self._index[(pred, None, None)]={s : 1}
        else: addIt(s, list)

        list = self._index.get((pred, None, obj), None)
        if list is None: self._index[(pred, None, obj)]={s : 1}
        else: addIt(s, list)

        list = self._index.get((pred, subj, None), None)
        if list is None: self._index[(pred, subj, None)]={s : 1}
        else: addIt(s, list)

        list = self._index.get((pred, subj, obj), None)
        if list is None: self._index[(pred, subj, obj)]={s : 1}
        else: addIt(s, list)
	...
	if F.canonical != None:
        ...
        F._existentialVariables = F._existentialDict.keys()
...
#	    self._existentialVariables.remove(bnode)
	...
	while bindings != {}:
	    ...
	    newBindings = {}
	    for s in self.statements.keys():  # take a copy!
		changed = 0
...
# Must be done by caller.
	...
	"""
	total = 0
	for t in context.statementsMatching(subj=subj).keys():
		    context.removeStatement(t)    # SLOW
		    total = total + 1
	for t in context.statementsMatching(pred=subj).keys():
		    context.removeStatement(t)    # SLOW
		    total = total + 1
	for t in context.statementsMatching(obj=subj).keys():
		    context.removeStatement(t)    # SLOW
		    total = total + 1
...
# 	return s[CONTEXT].removeStatement(s)
	...
	for s in context.statements.keys():
	    for p in PRED, SUBJ, OBJ:
		x = s[p]
