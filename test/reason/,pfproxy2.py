    ...
    def handle_response(self, server):
        ...
        if status == '401' and shead.has_key('policy'):
            ...
            if not headers.has_key('x-cred'):
                ...
                policy = shead['policy']
                proof = self.server._engine.findProof(self._addr, policy, 'judy-passwd') 
                #proof = '@@big-hairy-proof'
                headers['x-cred'] = "yes"
                content = proof.n3String()
...
# So that we can load n3 from a string into a store
...
def _testRein():
    ...
    #log(pf.n3String())
    log("Done.\n")
    log("Now checking proof...")

    # Check will not work with diag == 1
    diag.tracking = 0
    diag.setTracking(0)

    #log("result: %s" % check.valid(pf)) #@@ why does this not work

    log("result: %s" % check.valid(from_string(pf.n3String())))
    log("Done.\n")
